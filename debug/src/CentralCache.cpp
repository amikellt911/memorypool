...
{
    LogDebug("[CentralCache:fetchRange] 开始获取内存范围，索引: " + std::to_string(index) + 
    ...
    try 
    {
        ...
        if (!result)
        {
            ...
            LogDebug("[CentralCache:fetchRange] 从页缓存获取内存成功，共 " + 
            ...
            if (totalBlocks > allocBlocks)
            {
                ...
                *reinterpret_cast<void**>(start + (totalBlocks - 1) * size) = nullptr;
                ...
                LogDebug("[CentralCache:fetchRange] 剩余 " + std::to_string(totalBlocks - allocBlocks) + 
                                       " 块存入中心缓存");
            }
        } 
        else // 如果中心缓存有index对应大小的内存块
        {
            ...
            LogDebug("[CentralCache:fetchRange] 从中心缓存获取 " + 
